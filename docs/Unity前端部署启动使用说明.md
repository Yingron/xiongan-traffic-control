...
若同时启动 `llama-server:8081`，右侧“AI 云脑诊断”面板会直接从同一份正式 660 维快照
生成路口诊断文本并调用 `POST /api/v1/llm/analyze`。首次收到状态后自动诊断，之后每 60 秒
一次；“立即诊断”和“诊断 J25”仍可手动触发。云脑分析只展示事件与建议，不会修改 DQN 动作
或 SUMO 信号相位。没有启动 LLM 服务时，面板会显示结构化错误，DQN 闭环仍会继续运行。
