...
模型 ID：`shared-dqn-real-offpeak-perf-1m-v1`。

本机权重为平峰共享 DQN 100 万步检查点，实际输入是每路口 22 维；它不将动作掩码作为神经网络输入。API 仍会返回真实 `30 × 4` 需求门控掩码供 Dashboard 显示，并已支持后续 26 维掩码模型在推理时自动追加掩码。不得把当前 22 维模型宣称为已训练的掩码模型。
