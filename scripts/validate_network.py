...
def main():
    ...
    report=['# SUMO 接口兼容验证报告','',f'- 受控路口：30/30（J01-J30）',f'- 每路口相位：统一 4 个',f'- 路口类型：全部 traffic_light',f'- N/S/E/W 进口映射：30/30（边界路口≥3方向）',f'- 后端动作数组：固定顺序 J01 → J30，长度 30',f'- 动作映射：0→phase0，1→phase1，2→phase2，3→phase3','','## 结论','','**通过。该路网满足 30 路口、统一 4 动作和 N/S/E/W 状态提取接口要求。**']
